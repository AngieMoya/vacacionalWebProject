...
function RegisterRecruiter({ isLoggedIn }) {
  ...
  function Submit() {
    ...
    setError("");
    let enterprise_id = localStorage.getItem("token")
    ...
    Axios.post(
      "https://w5vxmb3jjf.execute-api.us-east-2.amazonaws.com/dev/users",
      userData
    )
      .then((res) => {
        if (res.data.status) {
      ...
      >
        <div className="login-form">
          <div className="login-header">
            ...
            <p>
              Ingrese un email y una contraseña para generar un nuevo reclutador
            </p>
          </div>
          <div className="form">
            ...
            <button className="button p-btn" onClick={Submit}>
              Registrar
            </button>
          </div>
        </div>
